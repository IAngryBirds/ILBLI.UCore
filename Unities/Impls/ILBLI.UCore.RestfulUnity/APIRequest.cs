...
{
    public class APIRequest : IRestful
    {
        ...
        /// <returns></returns>
        public virtual async Task<string> GetMessage(string methodKey, string postMsg)
        {
            string requestUri = _httpRequestConfig.GetMethodKeyUri(methodKey);
            requestUri =string.Format(requestUri,postMsg);
        ...
        protected async Task<string> SendMessage(HttpMethod method,string requestUri, StringContent content)
        {
            ...
            _uLog.Info($"{guidStr}:请求地址：{requestUri};请求方式：{method.ToString()};请求参数：{content.ReadAsStringAsync()?.Result}");
